...
describe("AREAS", () => {
  ...
  it("草原の異常個体出現率が0.05%", () => {
    expect(AREAS[0].abnormalRate).toBeCloseTo(0.0005, 6);
  });

  it("魔王城の異常個体出現率が1.8%", () => {
    expect(AREAS[7].abnormalRate).toBeCloseTo(0.018, 6);
  });
});
...
describe("EVENT_PROBABILITY_TABLE", () => {
  ...
  it("各エリアの確率合計が1になる", () => {
    for (let i = 1; i <= 8; i++) {
      ...
      expect(t.battle + t.rest + t.treasure + t.trap).toBeCloseTo(1, 5);
    }
  });
});
